...
class evaluator:
    def __init__(self, pred_csv_path=None, results_df=None):
        if pred_csv_path is None and results_df is None:
            raise ValueError("Either pred_csv_path or results_df must be provided.")
        
        # We initialize these here so they don't reload on every single row
    ...
    def evaluate_full_text(self, output_path=None, pred_csv_path=None, truth_csv_path=None, merged_df=None, pred_col="answer", truth_col="correct_answer", sources_col="sources"):
        """
        Evaluates generative free-text responses using BLEU, ROUGE, BERTScore, and Citation Recall.
        Maintains the exact same loading structure as evaluate_results to prevent breaking existing pipelines.
        """
        if merged_df is not None:
            df = merged_df
        elif pred_csv_path is not None and truth_csv_path is not None:
            print(f"Loading predictions from {pred_csv_path}...")
            pred_df = pl.read_csv(pred_csv_path)
            
            print(f"Loading ground truth from {truth_csv_path}...")
            truth_df = pl.read_csv(truth_csv_path)
            
            print("Merging datasets on the 'id' column...")
            # An inner join guarantees we only evaluate rows that exist in both datasets
            df = truth_df.join(pred_df, on="id", how="inner")
        else:
            raise ValueError("You must provide either 'merged_df' OR both 'pred_csv_path' and 'truth_csv_path'.")

        print(f"Starting Full-Text Evaluation for {df.height} rows...")

        def calculate_text_metrics(row):
            # Safe extraction of text, handling potential nulls
            pred_text = str(row[0]) if row[0] is not None else ""
            true_text = str(row[1]) if row[1] is not None else ""
            sources_text = str(row[2]) if row[2] is not None else ""

            # 1. CITATION RECALL: Does the generated text contain the required paragraph?
            true_cits = self.parse_citations(sources_text)
            if len(true_cits) > 0:
                # Normalize spaces to make substring matching robust
                pred_norm = " ".join(pred_text.split())
                hits = sum(1 for c in true_cits if " ".join(c.split()) in pred_norm)
                cit_recall = hits / len(true_cits)
            else:
                cit_recall = 0.0
            ...
            r_score = self.rouge_scorer.score(true_text, pred_text)
            rouge_l = r_score['rougeL'].fmeasure

            return {"cit_recall": float(cit_recall), "bleu": float(bleu), "rouge_l": float(rouge_l)}

        # Apply calculations using Polars map_rows
        print("Calculating BLEU, ROUGE, and Citation Recall...")
        metrics_df = df.select([pred_col, truth_col, sources_col]).map_rows(
            lambda row: calculate_text_metrics(row),
            return_dtype=pl.Struct([
                pl.Field("cit_recall", pl.Float64),
                pl.Field("bleu", pl.Float64),
                pl.Field("rouge_l", pl.Float64)
            ])
        ).unnest("map")

        # Merge new metrics back into the main DataFrame
        df = pl.concat([df, metrics_df], how="horizontal")

        # 4. BERTScore: Semantic Similarity (GPU Accelerated)
        print("Calculating BERTScore on GPU (this might take a minute)...")
        cands = df[pred_col].fill_null("").to_list()
        refs = df[truth_col].fill_null("").to_list()
        
        # lang="de" loads a model tuned for German text automatically
        _, _, F1 = score(cands, refs, lang="de", verbose=False)
        
        # Inject BERTScore tensor results back into Polars
        df = df.with_columns(pl.Series(name="bertscore_f1", values=F1.numpy()))

        # Aggregate Averages
        avg_cit_recall = df["cit_recall"].mean()
        avg_bleu = df["bleu"].mean()
        avg_rouge = df["rouge_l"].mean()
        avg_bertscore = df["bertscore_f1"].mean()

        print("\n" + "="*45)
        print(" FULL-TEXT EVALUATION RESULTS")
        print("="*45)
        print(f"Total Cases:            {df.height}")
        print(f"Avg Citation Recall:    {avg_cit_recall:.2%}")
        print(f"Avg BLEU Score:         {avg_bleu:.2%}")
        print(f"Avg ROUGE-L (F1):       {avg_rouge:.2%}")
        print(f"Avg BERTScore (F1):     {avg_bertscore:.2%}")
        print("="*45)

        if output_path is not None:
            df.write_csv(output_path)
            print(f"Detailed full-text metrics saved to: {output_path}")
